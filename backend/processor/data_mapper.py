...
class DataMapper:
    def __init__(self, scraped_data: Dict[str, Any], scraped_sources: Optional[Dict[str, str]] = None):
        self.scraped = scraped_data
        self.sources = scraped_sources or {}  # Map indicator_id -> source
    ...
    def map_all(self) -> Dict[str, Dict[str, Any]]:
        ...
        for ind_id, mappings in INDICATOR_MAP.items():
            ...
            if parts:
                ...
                # Get specific source if available, fallback to "scraped"
                specific_source = self.sources.get(ind_id, "scraped")
                result[ind_id] = {
                    "answer":     " | ".join(unique),
    ...
    def get(self, indicator_id: str) -> Optional[Dict[str, Any]]:
        # PRIORITY 1: Check for direct indicator ID match (from evidence processor)
        if indicator_id in self.scraped:
            value = str(self.scraped[indicator_id]).strip()
            if value and value not in ("", "None", "nan", "NaN", "No value"):
                # Get specific source if available, fallback to "scraped"
                specific_source = self.sources.get(indicator_id, "scraped")
                return {
                    "answer": value,
                    "confidence": 0.85,  # High confidence for direct matches
                    "source": specific_source,
                    "note": "Direct extraction from evidence source"
                }

        # PRIORITY 2: Use complex mapping for legacy data formats
