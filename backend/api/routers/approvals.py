...
from backend.database.models import ApprovalRequest, Company, Answer, User, EvidenceSource
...
def approve_request(
    ...
    # If it's a source approval, trigger evidence processing
    if req.type == "SOURCE":
        from backend.services.evidence_processor import process_evidence_background
        ...
        if evidence:
            # Trigger background processing (pass evidence ID, not db session)
            background_tasks.add_task(process_evidence_background, evidence.id)
