...
import { Upload, FileText, Link as LinkIcon, ExternalLink, X, Plus, Clock, AlertCircle, Loader } from 'lucide-react';
...
const EvidencePanel: React.FC<EvidencePanelProps> = ({ company, refreshKey }) => {
  ...
  // Poll for status updates when items are processing
  useEffect(() => {
  ...
  const handleSourceSubmit = async (source: PendingSource, file?: File) => {
    try {
      let ev;

      if (file) {
        // File upload path - use uploadEvidence endpoint
        ev = await api.uploadEvidence(company.id, file, source.tags[0], source.justification);
      } else {
        // URL path - use addEvidence endpoint
        ev = await api.addEvidence(company.id, {
          type: 'URL',
          name: source.value,
          tags: source.tags,
              ...
              <div className="flex items-start gap-3 overflow-hidden">
                <div className={`mt-1 w-8 h-8 rounded flex items-center justify-center shrink-0 ${
                  ...
                  {item.status === 'pending_review' ? <Clock className="w-4 h-4 animate-pulse" /> :
                   item.status === 'processing' ? <Loader className="w-4 h-4 animate-spin" /> :
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                      <p className={`text-sm font-medium truncate transition-colors ${
                        item.status === 'pending_review' ? 'text-amber-200' :
                        item.status === 'processing' ? 'text-blue-200' :
                        item.status === 'error' ? 'text-red-200' :
                      ...
                      {item.status === 'processing' && (
                          <span className="text-[10px] bg-blue-500/20 text-blue-400 px-1.5 py-0.5 rounded uppercase font-bold tracking-wide">
                              Processing
                          </span>
                      )}
                      {item.status === 'error' && (
                          <span className="text-[10px] bg-red-500/20 text-red-400 px-1.5 py-0.5 rounded uppercase font-bold tracking-wide">
                              Error
                          </span>
                      )}
                  </div>
